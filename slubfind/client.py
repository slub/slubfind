...
class SlubFind(Find):
    ...
    def app_search(self, query, qtype="default", facet={}, page=0, count=0, sort="", type_num=None, parser_class=None):
        return self.get_query(query, qtype=qtype, facet=facet, page=page, count=count, sort=sort,
                              data_format="app", type_num=type_num, parser_class=parser_class)

    def settings(self):
        url = self.url_query("qqqqq", qtype="default", facet={}, page=0, count=0, sort="",
                             data_format="json-all", type_num=None)
    ...
    def solr_params(self, query, qtype="default", facet={}, page=0, count=0, sort="", type_num=None):
        url = self.url_query(query=query, qtype=qtype, facet=facet, page=page, count=count, sort=sort,
                             data_format="json-solr-params", type_num=type_num)
    ...
    def solr_params_via_url(self, url, type_num=None):
        url = URLParser(url)
        if url.ok:
            return self.solr_params(url.query, qtype=url.qtype, facet=url.facets, page=url.page, count=url.count,
                                    sort=url.sort, type_num=type_num)

    def solr_request(self, query, qtype="default", facet={}, page=0, count=0, sort="", type_num=None):
        url = self.url_query(query=query, qtype=qtype, facet=facet, page=page, count=count, sort=sort,
                             data_format="json-solr-request", type_num=type_num)
